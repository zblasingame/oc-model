...
class AMOD:
    ...
    alpha : float, optional
        Ratio of anomalous data to normal data, default 1.
    anom_idx : int, optional
        Anomalous idx, default 0.
    debug : bool, optional
    ...
    def train(self, X, Y, low=None, high=None):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.n_epochs):
                ...
                n_anom = self.alpha * batch_x.shape[0]

                batch_anom_x = np.random.uniform(
                    low,
                    high,
                    (n_anom, self.n_features)
                )

                # batch_anom_x = .5 * ((high * np.random.randn(n_anom,
                # self.n_features) +
                # (low - 3*high)) + (high * np.random.randn(n_anom,
                # self.n_features) + (low + 3*high)))

                batch_x = np.concatenate((batch_x, batch_anom_x), axis=0)
                batch_y = np.concatenate(
                    (batch_y, np.ones(n_anom) * self.anom_idx),
                    axis=0
                )
    ...
    def train_acc(self, X, Y, X_test, Y_test):
        if self.normalize:
            _min = X.min(axis=0)
            _max = X.max(axis=0)
            a, b = self.normalized_bounds
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.n_epochs):
                ...
                batch_anom_x = np.random.uniform(
                    0.5 * ((2 + alpha) * a - (alpha * b)),
                    0.5 * ((2 + alpha) * b - (alpha * a)),
                    (n_anom, self.n_features)
                )

                _, loss = sess.run([self.opt, self.loss], feed_dict={
                    self.X: np.concatenate((batch_x, batch_anom_x), axis=0),
                ...
                # _, anom_loss = sess.run([self.opt, self.loss], feed_dict={
                #     self.X: batch_anom_x,
                #     self.Y: np.ones_like(batch_y) * self.anom_idx
                # })
                if not epoch % self.display_step:
                    ...
                    self.print((
                        'Epoch {0:04} | Loss {1:20.5f} | Accuracy {2:20.3f}'
                    ).format(epoch+1, loss, acc * 100))
