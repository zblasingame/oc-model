...
for i, exploit in enumerate(exploits):
    ...
    ex_data = []
    for j in range(5):
        with open('../data/raid/aae/{}/subset_{}/test_set.csv'.format(
            exploit, j
        ), 'r') as f:
...
layout = go.Layout(
    title='AAE Scores on 3 Time Step Anomaly Detection Normalized',
    yaxis=dict(title='Score'),
    boxmode='group'
)

fig = go.Figure(data=(norm, anom), layout=layout)
py.plot(fig, filename='sad/mnist/3ts_aae_scores_norm')
