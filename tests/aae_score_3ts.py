...
# Model creation
model = models.aae.model.AAE(
    n_epochs=100,
    batch_size=100,
    dataset='3ts_camel',
    debug=True,
    save_path='{}/.models/aae.ckpt'.format(main_dir)
)
...
for exploit in tqdm(exploits):
    for i in range(5):
        ...
        np.savetxt(
            '{}/data/raid/aae/{}/subset_{}/train_set.csv'.format(
                main_dir, exploit, i
            ),
        ...
        np.savetxt(
            '{}/data/raid/aae/{}/subset_{}/test_set.csv'.format(
                main_dir, exploit, i
            ),
